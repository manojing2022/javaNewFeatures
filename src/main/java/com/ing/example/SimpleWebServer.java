...
public class SimpleWebServer {
	...
	public static void main(String[] args) {
		...
		Predicate<Request> IS_GET = req -> "GET".equals(req.getRequestMethod());
		...
		var postHandler = HttpHandlers.handleOrElse(
			...
			(exchange) -> {
				...
				System.out.println("Headers Post:");
		...
		var getHandler = HttpHandlers.handleOrElse(
				IS_GET,
				(exchange) -> {
					...
					var responseBody = "Get handled successfully".getBytes();
					exchange.sendResponseHeaders(200, responseBody.length);
					exchange.getResponseBody().write(responseBody);
				},
				defaultHandler
